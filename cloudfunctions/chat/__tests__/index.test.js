...
describe("cloudfunction/chat", () => {
  ...
  it("sendMessage stores booking preview text for conversation list", async () => {
    const accessToken = "chat_send_booking_token";
    const tokenHash = crypto.createHash("sha256").update(accessToken).digest("hex");
    const currentUser = {
      _id: "user_doc_1",
      userId: "user_1",
    ...
    const messageAddMock = jest.fn().mockResolvedValue({ _id: "chat_message_3" });
    const conversationUpdateMock = jest.fn().mockResolvedValue({ stats: { updated: 1 } });
    const db = cloud.database();
    const originalImplementation = db.collection.getMockImplementation();
