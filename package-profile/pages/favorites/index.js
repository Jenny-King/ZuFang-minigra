...
Page({
  ...
  onLoad(options) {
    ...
    if (!authUtils.requireLogin({ redirect: true })) {
      logger.debug("favorites_onload_end", { blocked: "not_login" });
      return;
    }
    logger.debug("favorites_onload_end", {});
  },

  async onShow() {
    logger.debug("favorites_onshow_start", {});
    if (!authUtils.isLoggedIn()) {
      logger.debug("favorites_onshow_end", { blocked: "not_login" });
      return;
    }
    ...
    logger.debug("favorites_onshow_end", {});
  },
  ...
  async onPullDownRefresh() {
    logger.debug("favorites_pulldown_start", {});
    try {
      await this.refreshList();
    } finally {
      wx.stopPullDownRefresh();
      logger.debug("favorites_pulldown_end", {});
    }
  },

  async onReachBottom() {
    logger.debug("favorites_reach_bottom_start", { hasMore: this.data.hasMore });
    await this.loadMore();
    logger.debug("favorites_reach_bottom_end", {});
  },
  ...
  async refreshList() {
    logger.debug("favorites_refresh_start", {});
    this.setData({
      page: REQUEST_DEFAULT.PAGE,
      hasMore: true,
      openActionHouseId: ""
    ...
    logger.debug("favorites_refresh_end", {});
  },

  async loadMore() {
    logger.debug("favorites_load_more_start", {});
    if (this.data.loading || !this.data.hasMore) {
      logger.debug("favorites_load_more_end", { blocked: true });
      return;
    }
    await this.fetchList({ initial: false });
    logger.debug("favorites_load_more_end", {});
  },

  async fetchList({ initial }) {
    logger.debug("favorites_fetch_start", { initial });
    if (this.data.loading) {
      logger.debug("favorites_fetch_end", { blocked: "loading" });
      return;
    }
    ...
    try {
      ...
      logger.debug("api_resp", { func: "favorite.getList", code: 0 });
      ...
      logger.debug("favorites_fetch_end", { initial });
    }
  },
  ...
  async onCancelFavoriteTap(event) {
    logger.debug("favorites_cancel_start", { data: event.currentTarget.dataset || {} });
    const houseId = event.currentTarget.dataset.houseId;
    if (!houseId) {
      logger.debug("favorites_cancel_end", { blocked: "empty_house_id" });
      return;
    }
    try {
      logger.debug("api_call", { func: "favorite.toggle", params: { houseId } });
      ...
      logger.debug("favorites_cancel_end", {});
    }
  },

  onGoDetailTap(event) {
    logger.debug("favorites_go_detail_start", { data: event.currentTarget.dataset || {} });
    const houseId = event.currentTarget.dataset.houseId;
    if (!houseId) {
      logger.debug("favorites_go_detail_end", { blocked: "empty_house_id" });
      return;
    }
    navigateTo(ROUTES.HOUSE_DETAIL, { houseId });
    logger.debug("favorites_go_detail_end", { houseId });
  },

  applyTitleHighlight() {
    logger.debug("favorites_title_highlight_start", {});
    this.clearTitleHighlightTimer();
    ...
    logger.debug("favorites_title_highlight_end", {});
  },
